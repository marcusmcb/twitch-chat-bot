...
const createLiveReport = async (url) => {
  ...
  try {
    ...
    console.log("HERE");
	console.log(trackLog[trackLog.length - 1])
    ...
    let seratoLiveReport = {
      ...
      longest_track: {
		...
		time_since_played: timeDifference(trackLog[trackLog.length - 1].timestamp, trackLog[maxIndex].timestamp).split(":")[1],
      ...
      shortest_track: {
		...
		time_since_played: timeDifference(trackLog[trackLog.length - 1].timestamp, trackLog[minIndex].timestamp).split(":")[1],
